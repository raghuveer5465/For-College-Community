...
const RESOLVED_KEY = "foundItems_resolved_count";
...
document.getElementById("form").addEventListener("submit", function (e) {
    ...
    showToast("Found item report published!");
    renderSavedCards();
});
...
function formatDateTime(dateStr, timeStr) {
    ...
    return `Found on ${datePart} at ${timePart}`;
}
...
function CreateCard(data) {
        ...
        <div id="text">
            <div class="card-header-flex">
                ...
                <div class="card-actions">
                    ...
                    <button class="resolve-btn">Claimed!</button>
                </div>
            </div>
            ...
            <button class="contact-btn">📞 Claim Item</button>
        </div>
    `;

    // Contact Modal
    card.querySelector(".contact-btn").addEventListener("click", () => {
        showModal(`Claim from ${data.name}`, `Pickup Location: ${data.room}\nStaff/Student ID: ${data.studentId}`, true);
    });

    // Delete Event
    card.querySelector(".delete-btn").addEventListener("click", async () => {
        const confirmed = await showModal("Delete Report", "Are you sure you want to remove this found item report?");
        if (confirmed) {
            deleteCard(data.id);
    ...
    card.querySelector(".resolve-btn").addEventListener("click", async () => {
        const confirmed = await showModal("Item Claimed", "Has this found item been returned to its owner?");
        if (confirmed) {
